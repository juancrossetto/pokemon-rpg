...
function NavDrawerIcon({ src, active }: { src: string; active?: boolean }) {
  return (
    <span className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-md bg-white/4">
      <Image
        src={src}
        alt=""
        width={36}
...
function DrawerNavRow({
  ...
  const iconNode = item.iconSrc ? (
    ...
    <span
      className={`material-symbols-outlined flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-white/4 text-[20px]! ${
  ...
  const rowClass = `flex min-h-12 w-full items-center gap-3 rounded-xl px-2.5 py-2 transition-colors ${
    active
      ? "bg-pokeball-red/10 text-white"
      : "text-on-surface active:bg-white/[0.06]"
  }`;
      ...
      {badge > 0 ? (
        ...
        <span
          className={`material-symbols-outlined shrink-0 text-[18px]! ${
            active ? "text-pokeball-red/80" : "text-on-surface-variant/45"
          }`}
        >
          chevron_right
        </span>
...
export function MobileChrome({
  ...
  */
  ...
  const shortcuts = MOBILE_NAV_SHORTCUTS.map((id) => findNavItem(id)).filter(
    (item): item is NavItem =>
      item != null && !item.hidden && !item.disabled,
  );
  ...
  useEffect(() => {
    for (const groupId of MOBILE_BAR_GROUPS) {
      ...
      if (child) setLastNavHref(groupId, child.href);
    }
  }, [pathname, groups]);
  ...
  */
  ...
  }, [pathname, drawerShown, drawerFocusGroupId, primary.length, showMore]);
  ...
  */
  ...
  function renderGroupSections(groupsToShow: NavGroup[]) {
    return groupsToShow.map((group) => {
      ...
      return (
        <section key={group.id} data-nav-group={group.id} className="mb-4 last:mb-2">
          {!groupMode ? (
            <p className="mb-1.5 px-2.5 text-[10px] font-mono uppercase tracking-[0.18em] text-on-surface-variant/70">
            ...
            {items.map((item) => (
              <li key={item.id}>
                <DrawerNavRow
                  ...
                  label={navLabels.text[item.labelKey] ?? item.id}
                  description={navLabels.description[item.id]}
                  badge={item.badgeKey ? (navLabels.badges[item.badgeKey] ?? 0) : 0}
                  disabled={item.disabled}
                  soonLabel={navLabels.soon}
  ...
  const sheetTitle = groupMode
    ? (focusedGroup
        ? (navLabels.text[focusedGroup.labelKey] ?? focusedGroup.id)
        : moreLabel)
  ...
  return (
    <>
      ...
      */}
      <nav
        ref={bottomNavRef}
        className="fixed bottom-0 inset-x-0 z-[70] flex xl:hidden items-stretch min-h-[3.75rem] overflow-visible border-t border-white/10 bg-background/98 pb-[max(0.5rem,env(safe-area-inset-bottom,0px))] shadow-[0_-8px_24px_rgba(0,0,0,0.35)] backdrop-blur-xl"
      >
                  ...
                  {item.iconSrc ? (
                    ...
                    >
                      <Image
                        ...
                        priority
                      />
                      {item.groupId && active && !drawerShown ? (
                        <span
                          aria-hidden
                          className="material-symbols-outlined absolute -bottom-0.5 text-[12px]! text-pokeball-red/90"
                        >
                          expand_less
                        </span>
                      ) : null}
      ...
      {moreOpen && showMore && (
        <div className="fixed inset-0 z-[60] xl:hidden" role="presentation">
          <button
            ...
            className={`mobile-nav-sheet-backdrop absolute inset-x-0 top-0 bg-black/65 backdrop-blur-sm ${
              drawerPhase === "closing" ? "is-closing" : "is-open"
            }`}
            style={{ bottom: "var(--bottom-nav-h, 3.75rem)" }}
            onClick={closeMore}
          />
          <div
            ref={drawerRef}
            ...
            className={[
              "mobile-nav-sheet absolute inset-x-0 flex flex-col overflow-hidden rounded-t-2xl border-t border-white/12 bg-background/98 shadow-2xl backdrop-blur-xl",
              ...
              .join(" ")}
            style={{
              bottom: "var(--bottom-nav-h, 3.75rem)",
          ...
          >
            ...
            >
              {groups.length === 0 ? (
                <div className="flex flex-col items-center gap-3 px-4 py-10 text-center">
                  ...
                  <p className="text-sm text-on-surface-variant">{emptyNavLabel}</p>
                  <Link
                    href="/login"
                    onClick={closeMore}
            ...
            <div className="shrink-0 border-t border-white/10 bg-background/98 px-4 pt-3 pb-3">
              {groupMode && groups.length > 0 ? (
                <button
                  type="button"
                  onClick={() => openMore(null)}
                  className="flex min-h-11 w-full items-center justify-center gap-1.5 rounded-xl border border-white/12 bg-white/[0.03] text-sm font-medium text-on-surface active:bg-white/[0.07]"
                >
